    ...
    <div class="cart-list">
      <div class="pang-row" v-for="(item,index) in cartInfo" :key="index">
        ...
        <div class="pang-goods-price">
          ...
          <div>
            <van-button type="danger" size="mini" @click="showPopup(item.goodsId)">购买</van-button>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    showPopup(id) {
      ...
      this.cartInfo.map(item => {
        ...
        if (item.goodsId === id) {
          this.initialOrders.push(item);
          // this.cartInfo.slice()
          localStorage.setItem("ordered", JSON.stringify(this.initialOrders));
        }
      });
    }
  }
